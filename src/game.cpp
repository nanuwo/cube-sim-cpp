...
void Game::Draw() {
	...
	// Draw HUD
	DrawText(std::format("speed: {:.1f} globules per hobgoblin", m_currentSpeed).c_str(), 10, 10, 40, YELLOW);
	DrawText(std::format("x: {} y: {} z: {}", m_camera.target.x, m_camera.target.y, m_camera.target.z).c_str(), 10, 50, 40, YELLOW);
